...
Disclaimer:  
Don't try this with DosBOX or DosBOX-X this editor heavily uses floating point and by all means will be too slow in an emulation. You could install FreeDos on an USB stick using the application "Rufus". But be aware that USB sticks wear out if you do heavy/alot of saving. Better use a real (max 2TB) USB hard drive, you can use Rufus with it's "special" option enabled to do this (format 2TB and "burn" FreeDOS onto the hard drive). Also it's good to install SBEMUv1.5 to have it's memory manager (DPMI host).
